...
function profilListesi(fenomenler) {
  let fenomenlerYeni = [...fenomenler];
  for (let i = 0; i < fenomenlerYeni.length; i++) {
    fenomenlerYeni[i] = fenomenler[i].profile;
...
function fenomenSil(fenomenler, indeks) {
  let fenomenler2 = [...fenomenler];
  fenomenler2.splice(indeks, 1);
  return fenomenler2;
}
...
function fenomenEkle(fenomenler, number, profile, followers, posts, platform) {
  let fenomenlerDizisiYeni = [...fenomenler];
  let yeniFenomen = {
    number: 6,
    profile: "Workintech",
    followers: 10000000,
  ...
  fenomenlerDizisiYeni.push(yeniFenomen);
  return fenomenlerDizisiYeni;
}
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    const item = fenomenler[i];
    if (item.followers > 100000000) {
      enFenomenler.push(item.profile);
    }
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  let profileName = "";
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].platform === platform && typeof fenomenler[i].posts === "number" && fenomenler[i].posts > max) {
      max = fenomenler[i].posts;
      profileName = fenomenler[i].profile;
    }
  }
  return profileName;
}
let a = platformaGoreCokGonderiYapanFenomen(fenomenler, "TikTok");
console.log(a);
